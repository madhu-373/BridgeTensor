...
public:
     ...
     */
    virtual TensorBase operator()(const TensorBase& grad) = 0;
};
...
public:
    using hook_fn = std::function<TensorBase(const TensorBase&)>;

    explicit LambdaPreHook(hook_fn fn);
    TensorBase operator()(const TensorBase& grad) override;
...
public:
     ...
     */
    virtual void operator()(const TensorBase& grad) = 0;
};
...
public:
    using hook_fn = std::function<void(const TensorBase&)>;

    explicit LambdaPostAccHook(hook_fn fn);
    void operator()(const TensorBase& grad) override;
...
inline std::unique_ptr<FunctionPreHook> make_pre_hook(
    std::function<TensorBase(const TensorBase&)> fn) {
...
inline std::unique_ptr<PostAccumulateGradHook> make_post_acc_hook(
    std::function<void(const TensorBase&)> fn) {
